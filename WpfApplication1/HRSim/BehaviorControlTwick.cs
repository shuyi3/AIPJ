...
namespace HRSim
{
    ...
    public class BehaviorControlTwick : Behavior
    {
        //PenalityManager penman;

        public BehaviorControlTwick()
        {
            //penman = penman;
        }
        ...
        public override List<int> getPlayfieldFeature(Playfield p, bool own)
        {
            ...
            {
                featureList.Add(p.evaluatePenality); //twick
            }

            featureList.Add(mPlayer.owncards.Count * 5);

            featureList.Add(mPlayer.ownMaxMana);
            featureList.Add(ePlayer.ownMaxMana); //twick
            ...
            {
                featureList.Add(-3 * (aggroboarder + 1 - ePlayer.ownHero.Hp - ePlayer.ownHero.armor)); //twick
            }
        ...
        public override float getPlayfieldValue(Playfield p, bool own)
        {
            ...
            if (p.isOwnTurn != own)
            {
                retval += p.lastTurnEvaluatePenality; //twick
            }
            else
            {
            ...
            foreach (Minion m in mPlayer.ownMinions)
            {
                ...
                {
                    retval += 1;
                    if (!m.taunt && m.stealth) retval -= 20; //twick
                }
                else
                {
                ...
                if (m.handcard.card.name == CardDB.cardName.bloodmagethalnos) retval -= 10; //twick
                if (m.handcard.card.name == CardDB.cardName.nerubianegg)
                {
